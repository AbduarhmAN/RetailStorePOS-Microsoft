...
public sealed partial class CheckoutPage : Page, INotifyPropertyChanged
{
    ...
    public CheckoutPage()
    {
        ...
        this.FlowDirection = Loc.FlowDirection;
        ...
        ViewModel.SearchResults.CollectionChanged += SearchResults_CollectionChanged;
    ...
    private async void CheckoutPage_Loaded(object sender, RoutedEventArgs e)
    {
        ...
        try
        {
            // Native AOT explicit initial sync for collections bypasses x:Bind ItemsSource crashes
            SyncCollectionToItemsControl(ViewModel.CartItems, CartItemsListView.Items, new NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction.Reset));
            SyncCollectionToItemsControl(ViewModel.SearchResults, SearchResultsListView.Items, new NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction.Reset));
    ...
    private async void PrintReceiptButton_Click(object sender, RoutedEventArgs e)
    {
        try
        {
            ...
            if (!string.IsNullOrWhiteSpace(pdfPath) && File.Exists(pdfPath))
            {
                ...
                {
                    StartupTrace.Write($"CheckoutPage.PrintReceiptButton_Click shell print failed: {printEx}");
                    if (!ReceiptHelper.TryOpenReceiptPdf(pdfPath))
                    {
                        StartupTrace.Write("CheckoutPage.PrintReceiptButton_Click receipt PDF open fallback failed.");
                    }
                }
            }
        }
    ...
    private void CartItems_CollectionChanged(object? sender, NotifyCollectionChangedEventArgs e)
    {
        try
        {
            ...
            {
                SyncCollectionToItemsControl(ViewModel.CartItems, CartItemsListView.Items, e);
            });
        }
        catch (Exception ex)
        {
    ...
    private void SearchResults_CollectionChanged(object? sender, NotifyCollectionChangedEventArgs e)
    {
        try
        {
            DispatcherQueue.TryEnqueue(() => 
            {
                SyncCollectionToItemsControl(ViewModel.SearchResults, SearchResultsListView.Items, e);
            });
        }
        catch (Exception ex)
        {
            ReportCheckoutException(ex, "CheckoutPage.SearchResults_CollectionChanged");
        }
    }
    ...
    private void ViewModel_PropertyChanged(object? sender, PropertyChangedEventArgs e)
    {
        ...
        // Native AOT fix: set ReceiptItems ItemsSource from code-behind to
        // avoid the CsWinRT ABI marshalling crash in x:Bind compiled bindings.
        if (e.PropertyName == nameof(CheckoutViewModel.ReceiptItems))
        {
            DispatcherQueue.TryEnqueue(SyncReceiptItemsSource);
        }
    }

    /// <summary>
    /// Programmatically populates the receipt ItemsControl.
    /// Under Native AOT the CsWinRT projection of .NET generic
    /// collections to WinRT <c>IVector</c> can crash in
    /// <c>IItemsControlMethods.set_ItemsSource</c> when the x:Bind
    /// compiled binding goes through the ABI layer. Adding items
    /// directly to <c>ItemsControl.Items</c> sidesteps the
    /// collection projection entirely.
    /// </summary>
    private void SyncReceiptItemsSource()
    {
        try
        {
            ReceiptItemsControl.Items.Clear();
            var items = ViewModel.ReceiptItems;
            if (items is not null)
            {
                for (var i = 0; i < items.Count; i++)
                {
                    ReceiptItemsControl.Items.Add(items[i]);
                }
            }
        }
        catch (Exception ex)
        {
            ReportCheckoutException(ex, "CheckoutPage.SyncReceiptItemsSource");
        }
    }
