...
namespace RetailStorePOS.App.Modules.Products;

public sealed class ProductSearchService
{
    private static readonly ModuleContract OfflineLookupContract = CheckoutWorkflowContract.ProductLookupQuery;
    ...
    private void EnsureOfflineLookupReady()
    {
        if (!OfflineLookupContract.OfflineAllowed)
        {
            throw new InvalidOperationException($"Product lookup contract '{OfflineLookupContract.ContractKey}' must remain available offline.");
        }

        _ = CheckoutWorkflowContract.ResolveCatalogLookup(_repository);
    }
