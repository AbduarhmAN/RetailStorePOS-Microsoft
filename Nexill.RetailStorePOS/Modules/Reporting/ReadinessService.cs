namespace Nexill.RetailStorePOS.Modules.Reporting;
...
using global::RetailStorePOS.WinUiLogin;
    ...
    private void RegisterHandlers()
    {
        ...
        _runner.RegisterHandler(ReadinessScenarioKeys.CheckoutCompleteSale, async (profile, runId) => 
        {
            ...
            try
            {
                ...
                // Initialize dispatcher queue - might be null in headless testing, we assume CheckoutVM can handle it
                // We will test if the VM initializes and handles a basic property access
                var vm = new global::RetailStorePOS.WinUiLogin.ViewModels.CheckoutViewModel(
                ...
                result.Summary = $"Checkout engine verified. Found {totalSales} prior sales. VM initialized with Subtotal: {vm.Subtotal}";
            }
            catch (Exception ex)
            {
                result.Status = ScenarioStatus.Failed;
