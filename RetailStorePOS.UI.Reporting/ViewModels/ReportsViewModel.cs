...
namespace RetailStorePOS.UI.Reporting.ViewModels;
...
public sealed class ReportsViewModel : ObservableObject
{
    ...
    private async Task GenerateXReport()
    {
        try
        {
            ...
            var sales = await Task.Run(() => LoginRuntime.Sales.GetSalesByIds(run.SaleIds).OrderBy(sale => sale.CreatedAt).ToList());
            var storeName = LoginRuntime.Settings.GetStoreName() ?? LocalizationHelper.GetString("ReportsReceipts_StoreFallback");
            var periodLabel = LocalizationHelper.GetString("ReportsReceipts_Range_Today", "Today");
            var pdfPath = XReportHelper.GenerateXReport(sales, run.PeriodStartLocal, run.PeriodEndLocal, periodLabel, storeName);
