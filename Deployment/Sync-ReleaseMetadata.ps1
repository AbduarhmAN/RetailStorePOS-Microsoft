...
$identity.SetAttribute("Version", $metadata.PackageVersion)
...
$displayName.InnerText = "ms-resource:AppDisplayName"
$publisherDisplayName.InnerText = $metadata.BrandName
$visualElements.SetAttribute("DisplayName", "ms-resource:AppDisplayName")
$visualElements.SetAttribute("Description", "ms-resource:AppDescription")
...
if (-not $Quiet) {
    Write-Host "Synchronized WinUI manifest metadata to app version $($metadata.Version), binary version $($metadata.AssemblyVersion), package version $($metadata.PackageVersion)." -ForegroundColor Green
}
