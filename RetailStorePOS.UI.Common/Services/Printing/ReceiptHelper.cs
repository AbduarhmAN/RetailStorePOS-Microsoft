...
public static class ReceiptHelper
{
    ...
    public static bool OpenReceiptsFolder()
    {
        ...
        {
            StartupTrace.Write($"ReceiptHelper.OpenReceiptsFolder.ExplorerFailed:{explorerEx.Message}");
            try
            {
                Process.Start(new ProcessStartInfo
                {
            ...
            {
                StartupTrace.Write($"ReceiptHelper.OpenReceiptsFolder.ShellFailed:{folderEx.Message}");
                return false;
            }
        }
    }
    ...
    public static bool TryOpenReceiptPdf(string pdfPath)
    {
        ...
        {
            StartupTrace.Write($"ReceiptHelper.TryOpenReceiptPdf.LauncherFailed:{ex.Message}");
        }

        // Fallback for unpackaged development builds or when Launcher declines the file.
        try
        {
            Process.Start(new ProcessStartInfo
            {
        ...
        {
            StartupTrace.Write($"ReceiptHelper.TryOpenReceiptPdf.ShellFailed:{ex.Message}");
            return false;
        }
    }
